...
router.get('/directions', 
  ...
  async (req, res) => {
    ...
    try {
      ...
      const cachedResult = await BackendCacheManager.getCachedDirections(
        { lat: originLat, lng: originLng },
        { lat: destLat, lng: destLng }
      );

      if (cachedResult) {
        console.log('✅ Itinéraire récupéré du cache backend');
        return res.json({
          ...cachedResult,
          cached: true,
          cacheHit: true
      ...
      if (response.data.status === 'OK') {
        ...
        await BackendCacheManager.setCachedDirections(
          { lat: originLat, lng: originLng },
          { lat: destLat, lng: destLng },
          response.data
        );
...
router.get('/places/autocomplete', 
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Vérifier le cache d'abord
      const cachedResult = await BackendCacheManager.getCachedAutocomplete(input);
      if (cachedResult) {
        console.log('✅ Autocomplétion récupérée du cache backend');
        return res.json({
          ...cachedResult,
          cached: true,
          cacheHit: true
      ...
      if (response.data.status === 'OK') {
        // Sauvegarder en cache
        await BackendCacheManager.setCachedAutocomplete(input, response.data);
...
router.get('/places/details', 
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Vérifier le cache d'abord
      const cachedResult = await BackendCacheManager.getCachedPlaceDetails(place_id);
      if (cachedResult) {
        console.log('✅ Détails lieu récupérés du cache backend');
        return res.json({
          ...cachedResult,
          cached: true,
          cacheHit: true
      ...
      if (response.data.status === 'OK') {
        // Sauvegarder en cache
        await BackendCacheManager.setCachedPlaceDetails(place_id, response.data);
...
router.get('/cache/stats', async (req, res) => {
  try {
    const stats = BackendCacheManager.getStats();
    res.json({
      success: true,
      stats,
...
router.post('/cache/clear', async (req, res) => {
  try {
    await BackendCacheManager.flush();
    res.json({
      success: true,
